...
(() => {
  ...
  if (typeof VideoDecoder !== "undefined") {
    const reset = VideoDecoder.prototype.reset;
    VideoDecoder.prototype.reset = function () {
      const atMs = performance.now() - startedAt;
      if (atMs >= 0 && atMs < 30_000) {
        if (resets.length < 1000)
          resets.push({ atMs, queued: this.decodeQueueSize });
        else overflow += 1;
      }
  ...
  scope.__rustVideoObserver = {
    ...
    finish() {
      startedAt = Infinity;
      return { headers, resets, overflow };
    },
  };
