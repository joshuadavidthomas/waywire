...
test("observed resize keeps a full 1920 by 1080 desktop", () => {
  assert.deepEqual(__testing.fitObservedResize(1920, 1080), {
    width: 1920,
    height: 1080,
    downscale: 1,
  });
...
test("observed resize supports tall displays without exceeding its pixel budget", () => {
  ...
  assert.equal(fitted.width % 2, 0);
  assert.equal(fitted.height % 2, 0);
