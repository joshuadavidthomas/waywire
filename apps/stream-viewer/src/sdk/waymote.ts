...
type TransportPath = "/stream" | "/audio" | "/control";
...
const AudioConfigurationSchema = Schema.Struct({
  type: Schema.Literal("audio-config"),
...
const AudioPlayerMessageSchema = Schema.Union(
  Schema.Struct({
    type: Schema.Literal("underflow"),
    underflows: Schema.Number,
  }),
  Schema.Struct({
  ...
  Schema.Struct({
    ...
    captureTimestamp: Schema.NullOr(Schema.Number),
    captureFrame: Schema.NullOr(Schema.Number),
  }),
);
...
function createRuntime(owner: RuntimeOwner, options: WaymoteSessionOptions) {
  ...
  let audioReconnectTimer: Timer | null = null;
  let audioConnectAttempt: ConnectionAttempt | null = null;
  let audioConfiguration: AudioConfiguration | null = null;
  let audioDecoder: AudioDecoder | null = null;
  let audioDecoderSetupGeneration = 0;
  let audioContext: AudioContext | null = null;
  let audioPlayer: AudioWorkletNode | null = null;
  ...
  let audioVideoSkew: number | null = null;
  let audioScheduledEndFrame: number | null = null;
  ...
  let videoLateness = 0;
  let lastVideoCaptureMicros: number | null = null;
  ...
  function setAudioStatus(text: string): void {
    const state =
      text === "Audio active"
        ...
        : text === "Audio ready"
          ? "ready"
          : text.includes("unavailable") || text.includes("disabled")
            ? "unavailable"
            : text.includes("error") || text.includes("denied")
              ? "error"
              : text.includes("muted")
                ? "muted"
  ...
  function audioStartFrame(
    ...
    const outputTimestamp = currentAudioContext?.getOutputTimestamp?.();
    ...
    if (
      ...
      (outputContextTime === 0 && outputPerformanceTime === 0)
    ) {
      audioScheduledEndFrame = null;
      return null;
    }
    const contextTime =
      outputContextTime + (presentation - outputPerformanceTime) / 1000;
    const scheduledFrame = Math.round(contextTime * configuration.sampleRate);
    const earliestFrame = Math.ceil(
      (currentAudioContext.currentTime + 0.01) * configuration.sampleRate,
    );
    if (
      audioScheduledEndFrame !== null &&
      audioScheduledEndFrame - earliestFrame > configuration.sampleRate * 0.2
    ) {
      audioScheduledEndFrame = null;
    }
    const startFrame = Math.max(
      scheduledFrame,
      earliestFrame,
      audioScheduledEndFrame ?? 0,
    );
    if (startFrame > earliestFrame + configuration.sampleRate) {
      audioScheduledEndFrame = null;
      return null;
    }
  ...
  function closeAudioDecoder(): void {
    if (audioDecoder) {
      audioDecoder.close();
      audioDecoder = null;
    }
  ...
  async function configureAudioDecoder(): Promise<void> {
    if (
      !audioWanted ||
      !audioPlayer ||
    ...
    try {
      ...
      if (
        setupGeneration !== audioDecoderSetupGeneration ||
        sessionDisposed ||
        !sessionConnected
      )
        return;
    ...
    if (
      ...
      sessionDisposed ||
      !sessionConnected ||
      audioConfiguration !== sourceConfiguration
    )
      return;
    ...
    const configuredDecoder = new AudioDecoder({
      output(frame) {
        ...
        try {
          ...
          if (channels.length === 1 && mono) channels.push(mono.slice());
          const captureTimestamp = frame.timestamp;
          const startFrame = audioStartFrame(
            captureTimestamp,
  ...
  async function enableAudio(): Promise<void> {
    ...
    try {
      if (!audioContext) {
        ...
        const audioWorkletURL =
          options.audioWorkletURL ??
          (await import("./audio-player.ts?worker&url")).default;
        ...
        audioPlayer.port.onmessage = (event: MessageEvent<unknown>) => {
          const decoded = Schema.decodeUnknownEither(AudioPlayerMessageSchema)(
            event.data,
          );
          if (Either.isLeft(decoded)) return;
          const message = decoded.right;
          if (message.type === "underflow") {
            audioUnderflows = message.underflows;
          }
          if (message.type === "status" || message.type === "resync") {
            audioQueuedFrames = message.queuedFrames;
          }
          if (message.type === "status") {
            audioScheduled = message.scheduled;
            const currentConfiguration = audioConfiguration;
            const outputTimestamp = audioContext?.getOutputTimestamp?.();
            const outputContextTime = outputTimestamp?.contextTime;
            const outputPerformanceTime = outputTimestamp?.performanceTime;
            if (
              message.captureTimestamp !== null &&
              message.captureFrame !== null &&
              ...
              const audioPresentation =
                ...
                (message.captureFrame / currentConfiguration.sampleRate -
                  outputContextTime) *
                  1000;
              const audioAtVideoPresentation =
                message.captureTimestamp +
                (lastVideoPresentation - audioPresentation) * 1000;
              audioVideoSkew =
                (audioAtVideoPresentation - lastVideoCaptureMicros) / 1000;
            } else {
              audioVideoSkew = null;
            }
          }
      ...
      audioWanted = true;
      audioMuted = false;
      const currentGain = audioGain;
      const currentContext = audioContext;
      if (!currentGain || !currentContext)
        throw new Error("Audio graph was not created");
      currentGain.gain.value = audioVolume;
      ...
      audioStarting = false;
      if (!sessionDisposed && !audioConfiguration?.enabled)
        setAudioStatus("Session audio unavailable");
    }
  }
  ...
  function renderFrame(): void {
    ...
    if (now - lastStatsPublishedAt >= statsIntervalMilliseconds) {
      ...
      owner.setStats({
        ...
        audioVideoSkewMs: audioVideoSkew,
        ...
        decoderResets,
        audioQueueMs: audioConfiguration
          ? (audioQueuedFrames * 1000) / audioConfiguration.sampleRate
          : 0,
        audioUnderflows,
        audioMode: audioScheduled ? "sync" : "queue",
        resizeState,
      });
    }
    scheduleVideoPresentation();
  ...
  function decodeAudioMessage(buffer: ArrayBuffer): void {
    if (
      !(buffer instanceof ArrayBuffer) ||
      buffer.byteLength <= audioHeaderSize ||
      !audioDecoder ||
    ...
    if (
      ...
      audioDecoder.decodeQueueSize > 12
    ) {
      const configuration = audioConfiguration;
  ...
  async function connectAudio(): Promise<void> {
    ...
    socket.addEventListener("message", (event) => {
      ...
      if (typeof event.data === "string") {
        const message = decodeJson(AudioConfigurationSchema, event.data);
        if (!message) {
      ...
      if (event.data instanceof ArrayBuffer) decodeAudioMessage(event.data);
    });
    socket.addEventListener("close", () => {
      ...
      resetAudioPlayer();
      setAudioStatus(
        sessionConnected ? "Audio reconnecting" : "Audio disconnected",
      );
      if (sessionConnected && !document.hidden) {
        audioReconnectTimer = setTimeout(() => {
          audioReconnectTimer = null;
  ...
  function setMuted(muted: boolean): void {
    ...
    if (audioWanted)
      setAudioStatus(audioMuted ? "Audio muted" : "Audio active");
  }
...
export class WaymoteSession {
  ...
  constructor(options: WaymoteSessionOptions = {}) {
    ...
    this.audio = Object.freeze({
      ...
      get muted() {
        return session.#runtime.getAudioMuted();
      },
    });
